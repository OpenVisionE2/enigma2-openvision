...
class HdmiCec:
	...
	def messageReceived(self, message):
		if config.hdmicec.enabled.value:
			...
			print("[HdmiCec] messageReceived: address=%s  CECcmd=%s, cmd = %s, ctrl0=%s, length=%s \n" % (address, _CECcmd, cmd, ctrl0, length))
			...
			if cmd == 0x00: # feature abort
				if length == 0: # only polling message ( it's same as ping )
					print("[HdmiCec] eHdmiCec: received polling message")
				else:
	...
	def deepstandby(self):
		...
		powertimer = abs(NavigationInstance.instance.PowerTimer.getNextPowerTimeActive() - now) <= 360 or NavigationInstance.instance.PowerTimer.isProcessing(exceptTimer=0)
