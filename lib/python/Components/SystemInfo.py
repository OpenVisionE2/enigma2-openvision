...
from Tools.StbHardware import getWakeOnLANType, getBoxProc
...
SystemInfo["CRClockVoltage"] = model == "azboxminime"
...
SystemInfo["Fan"] = BoxInfo.getItem("fan") or fileCheck("/proc/stb/fp/fan") or exists("/proc/fan") or getBoxProc() in ("ultra", "premium+")
...
SystemInfo["LCDSupport"] = ("lcd" in displaytype or "lcd" in model) and not "text" in displaytype
SystemInfo["HiSilicon"] = socfamily.startswith("hisi") or exists("/proc/hisi") or fileCheck("/usr/bin/hihalt") or exists("/usr/lib/hisilicon")
SystemInfo["DefineSat"] = platform in ("octagonhisil", "octagonhisilnew", "gbmv200", "uclanhisil") or model in ("beyonwizv2", "viper4k")
SystemInfo["AmlogicFamily"] = socfamily.startswith(("aml", "meson")) or fileCheck("/proc/device-tree/amlogic-dt-id") or fileCheck("/usr/bin/amlhalt") or exists("/sys/module/amports")
