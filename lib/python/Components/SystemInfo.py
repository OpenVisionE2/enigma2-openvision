...
SystemInfo["canMultiBoot"] = SystemInfo["HasRootSubdir"] and (1, 4, "mmcblk0", False) or fileHas("/proc/cmdline", "_4.boxmode=") and (1, 4, "mmcblk0", False) or getBoxType() in ("gbue4k","gbquad4k") and (3, 3, "mmcblk0", True) or getBoxType() in ("osmio4k") and (1, 4, "mmcblk1", True)
SystemInfo["canMode12"] = fileHas("/proc/cmdline", "_4.boxmode=1 ") and '192M' or fileHas("/proc/cmdline", "_4.boxmode=12") and '192M'
SystemInfo["canFlashWithOfgwrite"] = not(getBoxType().startswith("dm") or getBoxType() in ("hd60","hd61") or SystemInfo["HasRootSubdir"])
...
SystemInfo["AmlogicFamily"] = getBoxType() in ("alien5","c300","c300pro","c400plus","k1plus","k1pro","k2pro","k2prov2","k3pro","kvim2","x8hp","odroidc2","wetekhub","wetekplay","wetekplay2","alien4")
