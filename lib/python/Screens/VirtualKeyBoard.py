...
from Components.MultiContent import MultiContentEntryPixmapAlphaBlend, MultiContentEntryPixmap, MultiContentEntryText
...
class VirtualKeyBoard(Screen, HelpableScreen):
	...
	def __init__(self, session, title=_("Virtual KeyBoard Text:"), text="", maxSize=False, visible_width=False, type=Input.TEXT, currPos=None, allMarked=False, style=VKB_ENTER_ICON, windowTitle=None):
		...
		self.cmds = {
			...
			u"SPACE": "self['text'].char(' '.encode('UTF-8', 'ignore'))",
			u"SPACEICON": "self['text'].char(' '.encode('UTF-8', 'ignore'))",
			u"SPACEICONALT": "self['text'].char(' '.encode('UTF-8', 'ignore'))"
		}
