...
class FactoryReset(Setup, ProtectedScreen):
	...
	def createSetup(self):
		...
		if not self.resetFull.value:
			...
			if len(self.skins):
				self.list.append((_("Remove all skin data"), self.resetSkins, _("Select 'Yes' to remove all user customizations of skin data. Selecting this option will remove all user based skin customizations. All affected skins will return to their standard settings. This will also clear any customized boot logos and backdrops.")))
			if len(self.timers):
	...
	def keySaveCallback(self, answer):
		...
		system("/usr/bin/showiframe /usr/share/bootlogo.mvi")
