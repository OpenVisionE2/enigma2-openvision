...
int eRTSPStreamClient::getOrbitalPosition(int fe, int src)
{
	...
	for (int i = 0; i < 144; i++)
	{
		...
		eDebug("[eRTSPStreamServer] %d: orbital position %d, m_slot_mask %d, lnNum %d, old OP %d", i, op1, lnb_param.m_slot_mask, lnb_param.m_satellites.size(), lnb_param.SatCR_idx); //lnb_param.LNBNum, lnb_param.old_orbital_position);
		if (current_src >= src)
		{
...
{
	eDebug("[eRTSPStreamServer] %s: operation %d, pid_str %s (len pids: %d)", __FUNCTION__, op, pid_str.c_str(), pids.size());
...
{
	eDebug("[eRTSPStreamServer] update pids called: %d pids in the queue", pids.size());
